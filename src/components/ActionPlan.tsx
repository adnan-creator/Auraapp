...
export default function ActionPlan({ onClose }: ActionPlanProps) {
  const [step, setStep] = useState<'select' | 'plan'>('select');
  const [selectedDate, setSelectedDate] = useState<Date | undefined>();
  const [eventName, setEventName] = useState('Product Launch Presentation');
  ...
  const checklistItems = [
    { day: '3 Days Before (Nov 19)', items: [
    ...
    { day: '2 Days Before (Nov 20)', items: [
  ...
  return (
    <div className="space-y-6">
      <DialogHeader>
        <DialogTitle>Your Plan for November 22</DialogTitle>
      </DialogHeader>
